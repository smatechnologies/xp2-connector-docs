...
description: 'Use maintain_logfiles.sh to delete XP2 Connector log files older than a specified number of days.'
tags:
  - Reference
...
`maintain_logfiles.sh` deletes XP2 Connector log files that are older than a configured number of days. The script is run on a schedule to prevent log files from accumulating indefinitely on the AIX platform.
...
```
run_xp_job-ccstmt1-20200227093236-13762792.log
```
:::
...
:::tip Example
The following shows the parameters as they appear in the `maintain_logfiles.sh` file:
```
#!/bin/sh
